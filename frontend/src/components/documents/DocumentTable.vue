...
const openDocument = computed(() => props.documents.find(document => document.id === openDocumentId.value) ?? null)
...
function handleDocumentAction(action: 'preview' | 'disable' | 'remove', document: DocumentRow): void {
  closeActionMenu()
  if (action === 'preview')
...
<template>
        ...
        <tr v-for="document in props.documents" :key="document.id">
          ...
          <td class="document-actions-cell">
            <div class="row-actions">
              <button class="row-actions-trigger" type="button" aria-label="打开文档操作" :aria-expanded="openDocumentId === document.id" @click.stop="toggleActionMenu($event, document)">
  ...
  <Teleport to="body">
    <div v-if="openDocument" class="row-actions-menu row-actions-menu-floating" :style="menuStyle" @click.stop>
      <template v-if="openDocument">
        <button type="button" @click="handleDocumentAction('preview', openDocument)">
          查看详情
        </button>
        <button v-if="openDocument.status === 'active'" type="button" :disabled="props.busyId === openDocument.id" @click="handleDocumentAction('disable', openDocument)">
          {{ props.busyId === openDocument.id ? '处理中…' : '停用文档' }}
        </button>
        <button v-if="openDocument.status !== 'deleted'" class="danger-action" type="button" :disabled="props.busyId === openDocument.id" @click="handleDocumentAction('remove', openDocument)">
          删除文档
        </button>
      </template>
    </div>
