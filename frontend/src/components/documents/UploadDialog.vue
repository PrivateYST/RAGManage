...
const fileLabel = computed(() => selectedFile.value ? `${selectedFile.value.name}（${formatSize(selectedFile.value.size)}）` : '选择要上传的文件')
  ...
  <div v-if="props.open" class="dialog-backdrop" @click.self="close">
    <form class="dialog-card upload-dialog" @submit.prevent="submit">
      ...
      <label class="upload-dropzone" :class="{ 'has-file': selectedFile }">
        ...
        <input ref="fileInput" type="file" accept=".md,.txt,.docx,.pdf" @change="validateFile(($event.target as HTMLInputElement).files?.[0])">
      </label>
