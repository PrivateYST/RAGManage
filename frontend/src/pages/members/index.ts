...
export function useMembersPage() {
  ...
  const activeMembers = computed(() => spaceMembers.value.filter(item => item.status === 'active'))
  ...
  async function loadPage(): Promise<void> {
    ...
    try {
      const requests: [Promise<{ items: KnowledgeBaseRow[] }>, Promise<{ items: SpaceMember[] }> | null, Promise<{ items: MembershipAuditItem[] }> | null] = [
      ...
      const [knowledgeBaseResult, memberResult, auditResult] = await Promise.all(requests.map(item => item ?? Promise.resolve({ items: [] })))
  ...
  async function changeKnowledgeBaseRole(member: KnowledgeBaseMember, roleCode: KnowledgeBaseRoleCode): Promise<void> {
    if (!selectedKnowledgeBaseId.value || member.role_code === roleCode)
      return
    clearNotice()
    busyId.value = `grant-${member.id}`
    try {
      await updateKnowledgeBaseMember(selectedKnowledgeBaseId.value, member.id, { role_code: roleCode })
