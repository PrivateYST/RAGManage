...
import { KNOWLEDGE_BASE_ROLE_LABELS } from '../../enum'

export const knowledgeBaseRoleOptions = Object.entries(KNOWLEDGE_BASE_ROLE_LABELS) as [KnowledgeBaseRoleCode, string][]
...
export function useKnowledgeBaseAccessPanel(
  ...
  const availableCandidates = computed(() => candidates.value.filter(item => item.grant_status !== 'active'))
