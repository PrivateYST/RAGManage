...
import { Eye, FileClock, Filter, RefreshCw, RotateCcw, Search, ShieldCheck } from 'lucide-vue-next'
...
function summaryText(item: AuditLogItem): string {
  ...
  if (!entries.length)
    return '没有附加变更摘要'
  const text = entries.slice(0, 3).map(([key, value]) => {
...
<template>
  <section class="page-section audit-page">
    <div class="page-intro">
      ...
      <button class="secondary-button" type="button" :disabled="loading || loadingMore" @click="load(true)">
    ...
    <form class="content-card audit-filter-card" @submit.prevent="load(true)">
      ...
      <label>
        ...
        <select v-model="filters.targetType">
          <option v-for="option in auditTargetOptions" :key="option.value" :value="option.value">{{ option.label }}</option>
        </select>
      </label>
      <label class="audit-actor-filter">
        <span>操作人</span>
        <div><Search :size="14" aria-hidden="true" /><input v-model.trim="filters.actor" maxlength="100" placeholder="姓名或登录名"></div>
      </label>
    ...
    <section class="content-card audit-table-card">
      <header>
        <div><span><ShieldCheck :size="16" aria-hidden="true" /></span><div><strong>不可变审计事件</strong><small>当前已加载 {{ items.length }} 条</small></div></div>
        <span class="status-pill ready">{{ filters.scope === 'space' ? '空间范围' : '平台范围' }}</span>
      </header>
      ...
      <div v-else class="audit-table-wrap">
        <table>
          <thead><tr><th>时间</th><th>操作人</th><th>操作</th><th>目标</th><th>变更摘要</th><th>范围</th><th>操作</th></tr></thead>
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td><time :datetime="item.created_at">{{ formatTime(item.created_at) }}</time></td>
              <td><strong>{{ item.actor_name || '系统' }}</strong><small>{{ item.actor_login || 'system' }}</small></td>
              <td><span class="audit-action-badge">{{ auditActionLabel(item.action) }}</span><code>{{ item.action }}</code></td>
              <td><strong>{{ auditTargetLabel(item.target_type) }}</strong><small>#{{ item.target_id || '—' }}</small></td>
              <td><p>{{ summaryText(item) }}</p></td>
      ...
      <footer v-if="items.length" class="audit-table-footer">
        ...
        <button v-if="nextCursor" class="secondary-button" type="button" :disabled="loadingMore" @click="load(false)">
