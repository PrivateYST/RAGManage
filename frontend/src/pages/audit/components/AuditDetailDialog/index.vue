  ...
  <div class="dialog-backdrop audit-detail-backdrop" @click.self="$emit('close')">
    <section class="dialog-card audit-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="audit-detail-title">
      <header class="dialog-heading">
        ...
        <button class="dialog-close" type="button" aria-label="关闭审计详情" @click="$emit('close')">
          <X :size="16" aria-hidden="true" />
        </button>
      </header>

      <dl class="audit-detail-grid">
        <div><dt>发生时间</dt><dd>{{ formatAuditTime(item.created_at) }}</dd></div>
        <div><dt>操作人</dt><dd>{{ item.actor_name || '系统' }}<small>{{ item.actor_login || 'system' }}</small></dd></div>
        <div><dt>所属范围</dt><dd>{{ item.tenant_name || '平台级' }}</dd></div>
        <div><dt>目标对象</dt><dd>{{ auditTargetLabel(item.target_type) }} · {{ item.target_id || '—' }}</dd></div>
        <div class="wide">
          <dt>操作标识</dt><dd><code>{{ item.action }}</code></dd>
        </div>
        <div class="wide">
          <dt>Request ID</dt><dd><code>{{ item.request_id || '未记录' }}</code></dd>
        </div>
      </dl>
