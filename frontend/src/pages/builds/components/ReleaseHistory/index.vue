...
import { Eye, History, Radio, RotateCcw } from 'lucide-vue-next'
  ...
  <section class="release-history content-card">
    ...
    <div v-else class="release-history-list">
      <article v-for="release in releases" :key="release.id" :class="{ active: release.is_active }">
        ...
        <div><strong>Release #{{ release.id }}</strong><small>构建 #{{ release.build_id }} · {{ release.document_count }} 份文档</small></div>
        <div><strong>{{ providerLabel(release.provider) }}</strong><small>Profile #{{ release.embedding_profile_id }} · {{ shortHash(release.embedding_definition_hash) }}</small></div>
        <div><strong>{{ release.model_name }}</strong><small>{{ release.dimension }} 维 · {{ shortHash(release.model_revision) }}</small></div>
        ...
        <div class="release-history-actions">
          ...
          <button v-if="release.rollback_available" class="release-rollback-button" type="button" @click="emit('rollback', release)">
