...
import { RotateCcw, TriangleAlert, X } from 'lucide-vue-next'
...
<template>
  <Teleport to="body">
    <div v-if="open" class="release-rollback-backdrop" @mousedown.self="handleClose">
      <section class="release-rollback-dialog" role="dialog" aria-modal="true" aria-labelledby="release-rollback-title">
        <header class="release-rollback-header">
          ...
          <button class="dialog-close-button" type="button" aria-label="关闭" :disabled="rollingBack" @click="handleClose">
        ...
        <div v-if="targetRelease && currentRelease" class="release-rollback-body">
          <section class="release-rollback-compare">
            <article>
              ...
              <small>构建 #{{ currentRelease.build_id }} · {{ currentRelease.document_count }} 份文档</small>
            </article>
            <RotateCcw :size="18" aria-hidden="true" />
            <article class="target">
              <span>回退目标</span>
              <strong>Release #{{ targetRelease.id }}</strong>
              <small>构建 #{{ targetRelease.build_id }} · {{ targetRelease.document_count }} 份文档</small>
            </article>
          </section>

          <section class="release-rollback-target">
            <div><span>嵌入模型</span><strong>{{ targetRelease.model_name }} · {{ targetRelease.dimension }} 维</strong></div>
            <div><span>接入配置</span><strong>{{ providerLabel(targetRelease.provider) }} · Profile #{{ targetRelease.embedding_profile_id }}</strong></div>
          </section>
          ...
          <section class="release-rollback-warning">
            ...
            <div>
              ...
              <p>如果目标 Release 中存在已删除、已停用、撤权或解析产物失效的文档，系统将拒绝回退。成功后，新的问答请求立即使用目标 Release。</p>
            </div>
          </section>
          ...
          <footer class="release-rollback-footer">
            ...
            <button class="primary-button" type="button" :disabled="!canConfirm" @click="handleConfirm">
              <RotateCcw :size="15" aria-hidden="true" />{{ rollingBack ? '正在回退…' : `确认回退到 #${targetRelease.id}` }}
            </button>
          </footer>
        </div>
      </section>
