...
import { CheckCircle2, GitCompareArrows, Rocket, TriangleAlert, X } from 'lucide-vue-next'
...
<template>
  <Teleport to="body">
    <div v-if="open" class="release-diff-backdrop" @mousedown.self="handleClose">
      <section class="release-diff-dialog" role="dialog" aria-modal="true" aria-labelledby="release-diff-title">
        <header class="release-diff-header">
          ...
          <button class="dialog-close-button" type="button" aria-label="关闭" :disabled="publishing" @click="handleClose">
        ...
        <div v-else-if="preview" class="release-diff-body">
          <section class="release-compare-grid">
            <article>
              ...
              <strong>{{ preview.current_release ? `Release #${preview.current_release.id}` : '尚未发布' }}</strong>
          ...
          <section class="release-validation" :class="preview.validation.ready ? 'ready' : 'failed'">
            <CheckCircle2 v-if="preview.validation.ready" :size="17" aria-hidden="true" />
            <TriangleAlert v-else :size="17" aria-hidden="true" />
            <div>
              <strong>{{ preview.validation.ready ? '发布前校验通过' : '发布前校验未通过' }}</strong>
              <p v-if="preview.validation.ready">
                文档、切片、向量、维度和空间范围均完整一致。
              </p>
              <ul v-else>
                <li v-for="item in preview.validation.errors" :key="`${item.code}-${item.document_id ?? ''}`">
          ...
          <section class="release-diff-counts">
            ...
            <article><span>未变化</span><strong>{{ preview.diff.counts.unchanged }}</strong></article>
          </section>

          <section class="release-config-compare">
            <div class="release-section-title">
              <strong>模型与接入配置</strong>
              <span :class="configChanged ? 'changed' : 'same'">{{ configChanged ? '配置有变化' : '配置未变化' }}</span>
            </div>
            <div class="release-config-grid">
              <span>候选 Profile</span><strong>#{{ preview.build.embedding_profile_id }} · {{ shortHash(preview.build.embedding_definition_hash) }}</strong>
          ...
          <section v-if="changedItems.length" class="release-document-diff">
            ...
            <div class="release-diff-list">
              <div v-for="item in changedItems" :key="item.document_id">
                <span class="change-badge" :class="item.change">{{ changeLabel(item.change) }}</span>
          ...
          <footer class="release-diff-footer">
            ...
            <div>
              <button class="secondary-button" type="button" :disabled="publishing" @click="handleClose">
                取消
              </button>
              <button class="primary-button" type="button" :disabled="!canPublish" @click="handlePublish">
