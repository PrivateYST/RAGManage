...
import { FileText, PackageCheck, X } from 'lucide-vue-next'
...
<template>
  <Teleport to="body">
    <div v-if="open" class="release-detail-backdrop" @mousedown.self="emit('close')">
      <section class="release-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="release-detail-title">
        ...
        <div v-else-if="release && detail" class="release-detail-body">
          <section class="release-explanation">
            ...
            <p>发布会把构建 #{{ release.build_id }} 中的文档版本、切片及向量配置冻结为不可变清单，并切换为知识问答当前检索版本。</p>
          </section>

          <section class="release-detail-metrics">
            <article><span>来源构建</span><strong>#{{ release.build_id }}</strong></article>
            <article><span>发布文档</span><strong>{{ detail.items.length }}</strong></article>
            <article><span>切片</span><strong>{{ totalChunks }}</strong></article>
            <article><span>向量</span><strong>{{ totalEmbeddings }}</strong></article>
          </section>

          <section class="release-detail-section">
            <div class="release-detail-section-title">
              <strong>发布配置</strong>
              <span class="status-pill" :class="release.is_active ? 'ready' : 'retired'">{{ release.is_active ? '当前使用' : '历史版本' }}</span>
            </div>
            <dl class="release-detail-config">
              <dt>发布时间</dt><dd>{{ formatDate(release.created_at) }}</dd>
              <dt>嵌入模型</dt><dd>{{ release.model_name }} · {{ release.dimension }} 维</dd>
              <dt>接入方式</dt><dd>{{ providerLabel(release.provider) }} · Profile #{{ release.embedding_profile_id }}</dd>
              <dt>接入端点</dt><dd>{{ release.base_url }}</dd>
              <dt>模型摘要</dt><dd>{{ shortHash(release.model_revision) }}</dd>
              <dt>配置摘要</dt><dd>{{ shortHash(release.embedding_definition_hash) }}</dd>
              <dt>清单摘要</dt><dd>{{ shortHash(release.manifest_hash) }}</dd>
              <dt>内容世代</dt><dd>Epoch {{ detail.build.input_epoch }}</dd>
            </dl>
          </section>
          ...
          <section class="release-detail-section release-document-list">
            ...
            <div v-if="detail.items.length" class="release-document-table">
              ...
              <div v-for="item in detail.items" :key="item.document_id" class="release-document-row">
