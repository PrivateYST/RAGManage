...
export function useEmbeddingProfileForm(endpoints: () => ModelEndpoint[]) {
  const embeddingEndpoints = computed(() => endpoints().filter(item => item.endpoint_type === 'embedding' && item.status === 'active'))
  const form = reactive({ endpointId: '', modelName: '', expectedDimension: 1024 })
  watch(embeddingEndpoints, (items) => {
    if (!items.some(item => item.id === form.endpointId))
      form.endpointId = items[0]?.id ?? ''
