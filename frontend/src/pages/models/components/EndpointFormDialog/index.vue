  ...
  <div v-if="open" class="dialog-backdrop" @click.self="emit('close')">
    <section class="dialog-card endpoint-dialog" aria-labelledby="endpoint-dialog-title">
      ...
      <p class="dialog-description">
        仅保存密钥引用；API Key 继续由服务端环境变量提供。
      </p>
      <form @submit.prevent="submit">
        <label>端点名称<input v-model="form.name" required maxlength="120"></label>
        <div class="endpoint-form-grid">
          <label>服务提供方<input v-model="form.provider" required :disabled="Boolean(endpoint)"></label>
          <label>模型用途<select v-model="form.endpointType" :disabled="Boolean(endpoint)"><option value="generation">生成模型</option><option value="embedding">嵌入模型</option><option value="reranker">重排模型</option></select></label>
        </div>
        <label>网关地址<input v-model="form.baseUrl" type="url" required placeholder="http://gateway.internal:8080"></label>
        <label>模型白名单<textarea v-model="form.modelsText" required placeholder="每行一个模型名称" /></label>
