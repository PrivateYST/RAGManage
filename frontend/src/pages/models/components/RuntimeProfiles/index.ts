...
export function useRuntimeProfileForm(endpoints: () => ModelEndpoint[]) {
  const generationEndpoints = computed(() => endpoints().filter(item => item.endpoint_type === 'generation' && item.status === 'active'))
  const form = reactive({
    embeddingProfileId: '',
