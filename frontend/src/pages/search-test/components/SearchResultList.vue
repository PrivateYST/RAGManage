...
import { AlertTriangle, CheckCircle2, Clock3, FileText, Filter, GitMerge, Hash, Layers3, MapPin, SearchX } from 'lucide-vue-next'
...
<template>
  <section v-if="searching" class="content-card search-result-loading" aria-live="polite">
    ...
    <div><strong>正在执行正式检索</strong><span>校验当前 Release，并融合向量语义与关键词候选…</span></div>
  </section>
  ...
  <template v-else>
    <section class="content-card search-trace-summary">
      ...
      <div class="search-trace-metrics">
        ...
        <article>
          <span>有效来源</span>
          <strong>{{ result.source_stats?.valid_documents ?? 0 }} / {{ result.source_stats?.total_documents ?? 0 }}</strong>
      ...
      <div v-if="result.filters.length" class="search-filter-list">
        ...
        <span v-for="filter in result.filters" :key="filter.name">
          {{ filter.name }}：{{ filter.value }}<em v-if="filter.filtered_count">过滤 {{ filter.filtered_count }}</em>
        </span>
      </div>
    </section>
    ...
    <section v-if="result.items.length" class="search-evidence-section">
      ...
      <div class="search-evidence-list">
        <article v-for="item in result.items" :key="item.chunk_id" class="content-card search-evidence-card">
          ...
          <div class="search-evidence-body">
            ...
            <footer>
              ...
              <span><Clock3 :size="12" aria-hidden="true" />向量排名 {{ item.vector_rank ?? item.raw_rank }}</span>
