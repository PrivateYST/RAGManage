...
const visibleDocuments = computed(() => {
  ...
  return documents.value.filter((document) => {
    ...
    const matchesStatus = statusFilter.value === 'all' || document.parse_status === statusFilter.value
    return matchesQuery && matchesStatus
  })
})

const activeKnowledgeBase = computed(() => knowledgeBases.value.find(item => item.id === activeKnowledgeBaseId.value))
...
async function loadDetailChunks(versionId: string): Promise<void> {
  ...
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      timeoutId = window.setTimeout(() => reject(new Error('切片预览请求超时，请检查后台解析任务状态')), 5000)
    })
...
<template>
  <section class="page-section documents-page">
    <div class="page-intro">
      ...
      <button class="primary-button" type="button" :disabled="!activeKnowledgeBaseId" @click="openUpload">
    ...
    <div class="content-card documents-context-card">
      <div class="documents-context-copy">
        ...
        <div>
          ...
          <small>{{ activeKnowledgeBase ? (purposeLabels[activeKnowledgeBase.purpose] || activeKnowledgeBase.purpose) : '登录用户暂无可访问知识库' }}</small>
        </div>
      </div>
      <label class="kb-select-label">
        <span>切换知识库</span>
        <select v-model="activeKnowledgeBaseId" :disabled="loadingKnowledgeBases" aria-label="切换知识库">
    ...
    <DocumentFilters v-model:query="searchQuery" v-model:status="statusFilter" :count="visibleDocuments.length" />

    <div v-if="loading" class="content-card module-placeholder document-loading">
      <span class="loading-spinner" />
      <p>正在加载文档…</p>
    </div>
    <DocumentTable v-else :documents="visibleDocuments" :busy-id="busyDocumentId" @preview="openDetail" @disable="handleDisable" @remove="handleRemove" />
  ...
  <UploadDialog :open="uploadOpen" :busy="uploading" :error="uploadError" @close="uploadOpen = false" @upload="handleUpload" />
  ...
  <div v-if="detail || detailLoading" class="dialog-backdrop" @click.self="closeDetail">
    <section class="dialog-card document-detail-dialog" aria-labelledby="document-detail-title">
      ...
      <div v-if="detailLoading" class="module-placeholder detail-loading">
        <span class="loading-spinner" /><p>正在读取版本和切片…</p>
      </div>
      <template v-else-if="detail">
        <div class="detail-meta">
          <span>状态：{{ detail.document.status === 'active' ? '启用' : detail.document.status === 'disabled' ? '停用' : '已删除' }}</span><span>版本数：{{ detail.versions.length }}</span>
        </div>
        <div class="version-list">
          <div v-for="version in detail.versions" :key="version.id" class="version-row">
            <div><strong>V{{ version.version_no }}</strong><small>{{ formatDate(version.created_at) }} · {{ version.file_size.toLocaleString() }} bytes</small></div>
            <span class="status-pill" :class="version.parse_status">{{ parseStatusLabel(version.parse_status) }}</span>
          </div>
        </div>
