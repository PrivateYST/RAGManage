...
export function createKnowledgeBase(payload: { tenant_id: number, name: string, description: string, purpose: string }): Promise<KnowledgeBaseRow> {
...
export function updateMenu(id: string, payload: Partial<Pick<MenuRow, 'name' | 'sort_order' | 'visible' | 'status'>>): Promise<MenuRow> {
...
export function updateUser(id: string, payload: { status?: 'active' | 'disabled', password?: string, display_name?: string }): Promise<UserRow> {
