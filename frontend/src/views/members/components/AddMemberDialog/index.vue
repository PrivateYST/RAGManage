...
<template>
  <AppDialog
    :open="open"
    title="添加成员"
    content-class="w-[min(460px,calc(100vw-2rem))]"
    @close="close"
  >
    <form
    ...
    >
      <div class="dialog-heading">
        <div>
          <p class="eyebrow">当前空间</p>
          <h2 id="add-member-title">添加成员</h2>
        </div>
        <button
          class="dialog-close"
          type="button"
      ...
      /></label>
      <label
        >空间角色<select v-model="roleCode">
          <option v-for="[value, label] in assignableSpaceRoles" :key="value" :value="value">
            {{ label }}
          </option>
        </select></label
      >
      <div class="member-role-help">
        ...
        <strong v-else-if="roleCode === 'customer_reader'"
          >可查看当前空间内全部已发布知识库并进行问答。</strong
        >
