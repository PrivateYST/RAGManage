...
async function submit(): Promise<void> {
  ...
  if (await auth.signIn(loginName.value.trim(), password.value)) await router.push('/dashboard')
}
</script>
  ...
  <main class="login-page">
    <section class="login-brand">
      ...
      <div class="login-value">
        <span>⌁</span>
        <div><strong>从资料到答案</strong><small>每个回答都能回到明确的原文证据</small></div>
      </div>
      <div class="login-value">
        <span>◈</span>
        <div><strong>空间级安全隔离</strong><small>客户资料、成员和会话相互独立</small></div>
      </div>
    </section>
