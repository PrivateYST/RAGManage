...
<template>
  <section class="content-card endpoint-card">
    <header class="endpoint-card-header">
      <div>
        <strong>模型端点</strong><small>密钥由环境变量注入，页面和数据库均不保存明文。</small>
      </div>
            ...
            <tr v-for="endpoint in items" :key="endpoint.id">
              ...
              <td>
                <select
                  ...
                  @change="
                    setSelectedModel(endpoint.id, ($event.target as HTMLSelectElement).value)
                  "
                >
              ...
              <td>
                <span class="health-state" :class="endpoint.health_status">{{
              ...
              <td>
                <span class="status-pill" :class="endpoint.status">{{
                  endpoint.status === 'active' ? '已启用' : '已停用'
                }}</span>
              </td>
