  ...
  <section class="content-card profile-section runtime-section">
    <header>
      <div>
        ...
        <div>
          <strong>Runtime Profile</strong><small>把检索、生成和回答规则固定为一次运行快照；激活后问答使用该版本。</small>
        </div>
      </div>
    ...
    <form
      ...
      "
    >
      <label>嵌入 Profile<select v-model="form.embeddingProfileId" required>
      ...
      <label>生成端点<select v-model="form.generationEndpointId" required>
        <option v-for="endpoint in generationEndpoints" :key="endpoint.id" :value="endpoint.id">
          {{ endpoint.name }}
        </option>
      </select></label>
      <label>生成模型<select v-model="form.generationModel" required>
        <option
      ...
      <div class="runtime-number-grid">
        <label>召回 Top K<input v-model.number="form.topK" type="number" min="1" max="20"></label><label>上下文字符<input
          v-model.number="form.contextMaxChars"
          type="number"
          min="1000"
          max="30000"
        ></label><label>温度<input v-model.number="form.temperature" type="number" min="0" max="2" step="0.1"></label>
      </div>
    ...
    <div class="profile-list runtime-list">
      <article v-for="item in items" :key="item.id">
        <div>
          <strong>#{{ item.id }} · {{ item.definition.generation?.model }} · Top
            {{ item.definition.retrieval?.top_k }}</strong><small>Embedding #{{ item.embedding_profile_id }} · 上下文
