  ...
  <section class="content-card profile-section">
    <header>
      <div>
        ...
        <div>
          <strong>Ingestion Profile</strong
          ><small>解析与切片规则形成不可变版本，新配置用于后续上传或重新解析。</small>
        </div>
      </div>
    ...
    >
      <label
        >单片最大字符<input v-model.number="form.maxChars" type="number" min="400" max="8000"
      /></label>
      <label
        >重叠字符<input
          ...
          :max="form.maxChars - 1"
      /></label>
      <button class="secondary-button" type="submit"><Plus :size="14" />创建新版本</button>
    </form>
    <div class="profile-list">
      <article v-for="item in items" :key="item.id">
        <div>
          <strong>#{{ item.id }} · {{ item.definition.chunking?.max_chars ?? 1800 }} 字符</strong
          ><small
            >{{ item.definition.parser }} · overlap
